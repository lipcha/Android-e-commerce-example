...
public final class Oauth1SigningInterceptor implements Interceptor {
    ...
    private Oauth1SigningInterceptor(String consumerKey, String consumerSecret, String accessToken,
                                     String accessSecret, Random random, Clock clock, PreferenceStorage preferenceStorage) {
...
//                throw new IllegalStateException("Key with no value: " + body.readUtf8());
        ...
        final String accessToken = preferenceStorage.getAccessTokenString();
        final String requestUrl = request.url().encodedPath();

        final String language = preferenceStorage.getCurrentLanguage();
        final String local = language.equals(Constants.LANGUAGE_DEFAULT) ? "default" : "en";
        final Request.Builder requestBuilder = request
                .newBuilder()
                .addHeader("store", local)
                .addHeader("Set-Cookie", "store=" + local);

        if (!TextUtils.isEmpty(accessToken)){
            for (String r : Constants.API.TOKEN_SIGN_IN_REQUESTS){
                if (requestUrl.contains(r))
                    return requestBuilder.addHeader("Authorization", BEARER + " " + accessToken).build();
            }
        }
        ...
        return requestBuilder.addHeader("Authorization", authorization).build();
    }
    ...
    public static final class Builder {
        ...
        public Builder storage(PreferenceStorage preferenceStorage){
            this.preferenceStorage = preferenceStorage;
            return this;
        }
        ...
        public Oauth1SigningInterceptor build() {
            ...
            return new Oauth1SigningInterceptor(consumerKey, consumerSecret, accessToken, accessSecret,
                    random, clock, preferenceStorage);
        }
    }
